...
public static class TCommon
{
    ...
    public static Vector3 RotateDirection(this Vector3 Direction, Vector3 axis, float angle,bool clockWise=true) =>(clockWise?1:-1)*( Quaternion.AngleAxis(angle, axis) * Direction).normalized;
    ...
    {
        return seed != null ? seed.Next(ir.start, ir.end + 1) : UnityEngine.Random.Range(ir.start, ir.end + 1);
    }
    public static float Random(this RangeFloat ir, System.Random seed = null)
    {
        return seed != null ? seed.Next((int)(ir.start*1000), (int)(ir.end*1000))/100 : UnityEngine.Random.Range(ir.start, ir.end + 1);
    }
}
