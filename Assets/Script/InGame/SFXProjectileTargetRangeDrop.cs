...
public class SFXProjectileTargetRangeDrop : SFXProjectile {
    ...
    protected override PhysicsSimulator GetSimulator(Vector3 direction, Vector3 targetPosition) => new ProjectilePhysicsLerpSimulator(transform, transform.position, targetPosition+Vector3.up*F_DropStartHeight, OnAnimFinished, Vector3.Distance(transform.position, targetPosition) / F_Speed, F_Height, F_Radius, GameLayer.Physics.I_All, OnPhysicsCasted);
