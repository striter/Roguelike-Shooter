...
public class EntityCharacterAI : EntityCharacterBase {
    ...
    class EnermyAIControllerBase : ISingleCoroutine
    {
        ...
        bool FrontBlocked()
        {
            Debug.DrawRay(headTransform.position,headTransform.forward*2,Color.red,.3f);
            Debug.Log(m_Entity.B_AttackFrontCheck && Physics.SphereCast(new Ray(headTransform.position, headTransform.forward), 1f, 2, GameLayer.Mask.I_Static));
return m_Entity.B_AttackFrontCheck && Physics.SphereCast(new Ray(headTransform.position, headTransform.forward), 1f, 2, GameLayer.Mask.I_Static);
        } 
