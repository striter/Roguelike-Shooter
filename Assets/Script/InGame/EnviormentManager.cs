    ...
    public void SetPostEffects(enum_Style _levelStyle)
    {
        ...
        switch (_levelStyle)
        {
            case enum_Style.Undead:
                PostEffectManager.AddPostEffect<PE_FogDepthNoise>().SetEffect(Color.white).SetEffect(TCommon.ColorAlpha(Color.white, .5f), .4f, -1f, 2f).SetTexture(TResources.Load<Texture>("Texture/Noise_Fog"));
                break;
        }
    }
    #region Level
