...
public class UIC_GameMinimap : UIControlBase {
    ...
    void OnStageStart()
    {
        ...
        m_Style.text = string.Format("<color=#ffffff88>{0}</color>:<color=#fe9e00>{1}</color>", TLocalization.GetKeyLocalized("UI_MAP_STYLE"), TLocalization.GetKeyLocalized(GameManager.Instance.m_GameLevel.m_GameStyle.GetLocalizeKey()));
    }
