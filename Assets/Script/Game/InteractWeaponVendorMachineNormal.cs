...
public class InteractWeaponVendorMachineNormal : InteractGameBase {
    ...
    protected override bool OnInteractedCheck(EntityCharacterPlayer _interactor)
    {
        ...
        GameObjectManager.SpawnInteract<InteractWeaponPickup>(transform.position+TCommon.RandomXZCircle() , Quaternion.identity).Play(WeaponSaveData.CreateNew(GameDataManager.m_WeaponRarities[TCommon.RandomPercentage(GameConst.D_EventWeaponVendorMachineNormalRate)].RandomItem()));
        return true;
    }
}
